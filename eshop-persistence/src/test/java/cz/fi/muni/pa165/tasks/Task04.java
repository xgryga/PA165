...
public class Task04 extends AbstractTestNGSpringContextTests {
	...
	public void findProductByName(){
		List<Product> found = em.createQuery("select p from Product p where p.name like :name",Product.class).setParameter("name", "Flashlight").getResultList();
	...
	public void findProductsAddedYesterday() {
		...
		List<Product> products= em.createQuery("select p from Product p where p.addedDate = :date",Product.class).setParameter("date", cal.getTime().toInstant().atZone(ZoneId.systemDefault()).toLocalDate()).getResultList();
	...
	public void leftJoinCagesWithPets() {
		...
		List<ProductAndCategory> productAndCategory = em.createQuery("select new cz.fi.muni.pa165.tasks.ProductAndCategory(p, c) from Category c LEFT JOIN Product p ON c.id = c.products.id",ProductAndCategory.class).getResultList();
	...
	public void criteriaFindProductsWithNonEmptyCategory() {
		...
		//content of where should use CriteriaBuilder.isNotEmpty method
                ...
                Root<Product> q = query.from(Product.class);
                query.select(q).where(cb.isNotEmpty(q.get("categories")));
